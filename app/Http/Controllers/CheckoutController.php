...
class CheckoutController extends Controller
{
    ...
    public function checkout(Request $request)
    {
        ...
        if (auth()->check()) {
            ...
            // For guest checkout, get user from session
            $guest_user_id = Session::get('guest_user_id');
            if (!$guest_user_id) {
                flash(translate('Guest user session expired. Please try again.'))->warning();
                return redirect()->route('checkout');
            }
    ...
    public function createUser($guest_shipping_info)
    {
        ...
        // Guest Account Opening and verification(if activated) email send - only if email provided
        if (!empty($user->email)) {
            try {
                EmailUtility::customer_registration_email('registration_from_system_email_to_customer', $user, $password);
            } catch (\Exception $e) {
                // Don't delete user if email fails, as email is optional
            }
        ...
        $address->country_id    = !empty($guest_shipping_info['country_id']) ? $guest_shipping_info['country_id'] : 18; // Default to Bangladesh
        $address->state_id      = $guest_shipping_info['state_id'] ?? null;
        $address->city_id       = !empty($guest_shipping_info['city_id']) ? $guest_shipping_info['city_id'] : null;
        $address->postal_code   = $guest_shipping_info['postal_code'] ?? null;
        $address->area_id       = $guest_shipping_info['area_id'] ?? null;
        $address->phone         = isset($guest_shipping_info['country_code']) ? $guest_shipping_info['country_code'].$guest_shipping_info['phone'] : '+880'.$guest_shipping_info['phone'];
