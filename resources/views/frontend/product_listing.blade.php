                        ...
                        @if(get_setting('color_filter_activation') && count($colors) > 0)
                            <div class="filter-item color-filter">
                                ...
                                <div class="color-options">
                                    ...
                                    @endforeach
                                    @if($colors->count() > 8)
                                        <div class="color-more-btn" onclick="toggleMoreColors(this)">+{{ $colors->count() - 8 }}</div>
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-8">
                            ...
                            <p class="results-count">
                                <span id="product-count">{{ $products->total() }}</span> products found
                            </p>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (priceSliderElement) {
            ...
            const currentMax = parseFloat(priceSliderElement.dataset.currentMax);
    ...
    function toggleMoreColors(button) {
        ...
        if (moreColorsContainer && moreColorsContainer.classList.contains('more-colors')) {
            if (moreColorsContainer.style.display === 'none') {
                ...
                // Recalculate and update count text if needed, otherwise 'More'
                const count = button.dataset.count || 0; // If you stored count in data-count
                const totalColors = @json($colors->count());
                const displayedColors = 8;
                button.textContent = `+${totalColors - displayedColors}`;
            }
        }
    }
    ...
    function toggleMoreAttributes(button, attributeId) {
        ...
        if (moreAttributesContainer && moreAttributesContainer.classList.contains(`attribute-${attributeId}`)) {
            if (moreAttributesContainer.style.display === 'none') {
                ...
                const totalValues = @json($attributes->keyBy('id')->map(fn($attr) => $attr->attribute_values->count()));
                const displayedValues = 4;
                button.textContent = `+${totalValues[attributeId] - displayedValues}`;
            }
        }
    }
